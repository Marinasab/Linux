...
CURRENT_CSV=""  # Track the current CSV file
...
analyze_species_gender() {
    if verify_csv; then
        ...
        awk -F',' 'NR>1 && $2 == "'"$species"'" && $3 == "'"$gender"'" {print $0}' "$CURRENT_CSV" | tee -a "$OUTPUT_LOG"
        avg_weight=$(awk -F',' 'NR>1 && $2 == "'"$species"'" && $3 == "'"$gender"'" {sum+=$4; count++} END {if (count > 0) printf "%.2f", sum/count}' "$CURRENT_CSV")
...
save_csv_copy() {
    if verify_csv; then
        read -p "Name the new CSV file for saving: " new_file
        cp "$CURRENT_CSV" "$new_file"
        log_entry "Saved data to new CSV: $new_file"
    fi
}
...
modify_weight() {
    if verify_csv; then
        ...
        target_line=$((row_num + 1))  # Add 1 to account for header
        sed -i "${target_line}s/\([^,]*,[^,]*,[^,]*,\)[0-9]*\.?[0-9]*/\1$new_weight/" "$CURRENT_CSV"
...
view_csv_list() {
    if verify_csv; then
        log_entry "Showing CSV content with line numbers from $CURRENT_CSV:"
        awk 'NR==1 {print $0; next} {print NR-1 ". " $0}' "$CURRENT_CSV" | tee -a "$OUTPUT_LOG"
        log_entry "Completed displaying CSV data"
    fi
}
...
analyze_species_weight() {
    if verify_csv; then
        ...
        awk -F',' 'NR>1 && $2 == "'"$species"'" {print $0}' "$CURRENT_CSV" | tee -a "$OUTPUT_LOG"
        avg_weight=$(awk -F',' 'NR>1 && $2 == "'"$species"'" {sum+=$4; count++} END {if (count > 0) printf "%.2f", sum/count}' "$CURRENT_CSV")
...
add_entry() {
    if verify_csv; then
        ...
        echo "$date,$species,$gender,$weight" >> "$CURRENT_CSV"
...
remove_row() {
    if verify_csv; then
        ...
        target_line=$((row_num + 1))  # Add 1 to account for header
        sed -i "${target_line}d" "$CURRENT_CSV"
...
verify_csv() {
    if [ -z "$CURRENT_CSV" ] || [ ! -f "$CURRENT_CSV" ]; then
        log_entry "Alert: No CSV file exists. Use option 1 to create one."
        return 1
    fi
    return 0
}
...
new_csv_file() {
    read -p "Choose a name for the CSV file: " file_name
    echo "Date collected,Species,Sex,Weight" > "$file_name"
    CURRENT_CSV="$file_name"
    log_entry "Generated new CSV file: $file_name with headers"
}
